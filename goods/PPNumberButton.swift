...
public typealias ResultInfoClosure = (_ number: Int,_ increase:Bool)->()
...
@IBDesignable open class PPNumberButton: UIView {
    ...
    var NumberResultIndoClosure: ResultInfoClosure?
    var IncreaseNumberResultClosure: ResultClosure? // 加法闭包
    ...
    @IBInspectable
    public var decreaseHide:Bool = false{
        didSet{
            if decreaseHide {
                ...
                if let text = textField.text {
                    if let value = Int(text) {
                        if value < _minValue {
                            ...
                            decreaseBtn.frame = CGRect.init(x: frame.width - frame.height, y: 0, width: frame.height, height: frame.height)
                        }else{
                            decreaseBtn.frame = CGRect.init(x: 0, y: 0, width: frame.height, height: frame.height)
                        }
    ...
    override open func layoutSubviews() {
        ...
        if let text = textField.text {
            if let value = Int(text) {
                if (decreaseHide && value < _minValue) {
                    decreaseBtn.frame = CGRect.init(x: width - height, y: 0, width: height, height: height)
                } else {
                    decreaseBtn.frame = CGRect.init(x: 0, y: 0, width: height, height: height)
                }
            }
        }
    }
...
extension PPNumberButton: UITextFieldDelegate {
    ...
    public func textFieldDidEndEditing(_ textField: UITextField) {
        if (textField.text?.characters.count)! == 0 || Int(textField.text!)! < _minValue {
            textField.text = decreaseHide ? "\(_minValue - 1)" : "\(_minValue)"
        }
...
public extension PPNumberButton {
    ...
    func increaseNumberResult(_ finished: @escaping ResultClosure) -> () {
        IncreaseNumberResultClosure = finished
    }
    func numberInfoResult(_ finished: @escaping ResultInfoClosure) -> () {
        NumberResultIndoClosure = finished
    }
